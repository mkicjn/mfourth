...
"`$2'","`AGAIN'","`$1,branch_code,LIT(m4_eval(-m4_count($1)-1)*sizeof(cell_t))'",
"`$2'","`UNTIL'","`$1,zbranch_code,LIT(m4_eval(-m4_count($1)-1)*sizeof(cell_t))'",
"`$1,zbranch_code,LIT(m4_eval(m4_count($2)+3)*sizeof(cell_t)),$2,branch_code,LIT(m4_eval(-m4_count($1,$2)-3)*sizeof(cell_t))'")'")
...
m4_addsubst("` 0>= '","` zgte_code, '")m4_dnl
m4_define("`m4_teststring'","`: TEST ( test ) IF DUP ELSE DROP THEN ;'")m4_dnl
